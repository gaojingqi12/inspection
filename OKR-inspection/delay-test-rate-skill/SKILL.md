...
description: 巡检 InsightEngine 延期提测率指标；页面自动化脚本只生成查询截图，JoyClaw 负责读取目标卡片数据、生成每日 JSON、读取历史 JSON 与生成本周趋势 JSON。
---
...
如果截图无法读取，JoyClaw 仍写入当天文件，`status` 设为 `failed`，`metrics` 中无法识别的值写 `null`，并增加 `error` 说明。
