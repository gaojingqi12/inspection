...
- 例外：`OKR-inspection/bi-weekly-delivery-rate-skill/scripts/run_skill.py` 会在查询后直接从页面图表 DOM / ECharts 实例读取双周交付率，并写入 `out/history/YYYY-MM-DD.json`。
- 除双周交付率外，其他 OKR skill 的 `scripts/run_skill.py` 不生成每日巡检 JSON，不读取历史 JSON，也不计算趋势。
- OKR 部分：JoyClaw 负责读取截图、识别指标、生成 `out/history/YYYY-MM-DD.json`、读取历史 JSON、生成 `out/weekly-trend-from-screenshot.json`；双周交付率优先直接读取脚本生成的每日 JSON，不再走视觉识别单点值。
...
- 持续交付部分：JoyClaw 负责读取 `ContinuousDelivery-inspection/out/three_cards.png`，只提取三张卡片中的三个指标，并生成 `ContinuousDelivery-inspection/out/continuous_delivery_YYYY-MM-DD.json`。
- 总编排脚本 `aggregate_report.py` 只读取 JoyClaw 已经生成的 JSON，产出总 JSON 和 HTML；它不负责从截图识别 OKR 指标，也不负责判断 AI 深度用户。
...
| 双周交付率 | 优先读取 `OKR-inspection/bi-weekly-delivery-rate-skill/out/history/YYYY-MM-DD.json`；截图仅作展示 |
...
- JoyClaw 优先读取 `out/history/` 中已有每日 JSON；双周交付率已由脚本直接生成当天 JSON。
...
- 不要再从 `OKR-inspection/bi-weekly-delivery-rate-skill/out/03_after_query.png` 做视觉识别提取当天单点值。
- 当天值只使用 `OKR-inspection/bi-weekly-delivery-rate-skill/scripts/run_skill.py` 直接写出的 `out/history/YYYY-MM-DD.json`。
- 如果缺少历史日期，只汇总已有日期，不要根据截图补历史值。
...
持续交付脚本生成截图后，JoyClaw 必须先生成当天 JSON，再运行总编排。不要让总报告直接读截图，也不要只在对话中给出数值。
...
如果当天持续交付 JSON 缺失，JoyClaw 必须回到 `ContinuousDelivery-inspection/SKILL.md` 的规则，先读取 `ContinuousDelivery-inspection/out/three_cards.png` 生成 JSON，再重新生成总 HTML。

## 成功标准

- OKR 4 个单项 skill 均已执行，或失败项有明确错误截图与错误信息。
- AI 巡检已执行，并且 JoyClaw 已生成或兜底读取当天非深度用户名单。
- 持续交付巡检已执行，并且 JoyClaw 已生成当天三指标 JSON。
