...
- `scripts/run_skill.py` 不生成每日巡检 JSON，不读取历史 JSON，也不计算趋势。
- OKR 部分：JoyClaw 负责读取截图、识别指标、生成 `out/history/YYYY-MM-DD.json`、读取历史 JSON、生成 `out/weekly-trend-from-screenshot.json`。
...
- JoyClaw 优先读取 `out/history/` 中已有每日 JSON；如果没有每日 JSON，必须从截图中的历史结果、趋势图、明细表或可见日期列直接抽取本周序列。
